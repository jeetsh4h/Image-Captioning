        ...
        for _ in range(max_seq_len):
            ...
            for beam in beams:
                ...
                for i in range(beam_size):
                    new_beams.append(
                        (
                            beam[0] + [top_k_tokens[i].item()],
                            beam[1] + top_k_scores[i].item(),
                        )
                    )
            ...
            if print_process:
                ...
                print(
                    f"Completed beams: {[tokenizer.decode(beam[0]) for beam in completed]}"
                )
...
def evaluate():
    ...
    for b in beam_size:
        print(
            "Beam size {}, prediction saved in {}".format(
                b, os.path.join(args.output_dir, f"prediction_beam_size_{b}.json")
            )
        )
        res = scores[f"beam{b}"]
